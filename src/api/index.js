...
const api = {
  ...
  getHomeRecommendData(){
    return http.postByNoLoading(`/Home/AppHomeRecommendPage?versionNo=4.5.8`)
  },
  //获取首页-推荐页猜你喜欢商品列表
  ...
  //获取首页-框架页数据
  getHomeFramesData(){
    return http.postByNoLoading(`/Home/AppHomeFramesPage?versionNo=4.5.8`)
  },
  //获取首页-框架页商品列表
  ...
  //获取首页-海淘页数据
  appHomeOverseasPage(){
    return http.postByNoLoading(`/home/AppHomeOverSeasPage?versionNo=4.5.8`)
  },
  //初始化推荐
  ...
  getOverSeasRecommendGoodsByPage({appHomeId,overseasModuleType ,pageSize = 10,pageIndex = 1}){
    ...
    //获取首页-美瞳页数据
    getHomeMeiTongData(){
      return http.postByNoLoading(`/home/AppHomeColoredContactsPage?versionNo=4.5.8`)
    },
    //获取首页-美瞳页商品列表
    getHomeMeiTongGoods(pageIndex, pageSize){
      let url = `/home/GetAppHomeColoredContactsGoodsByPage?versionNo=4.5.8&pageSize=${pageSize}&pageIndex=${pageIndex}`
      if(pageIndex == 1){
        return http.postByNoLoading(url)
      }
      return http.post(url)
    ...
    //获取首页-隐形护理页数据
    getHomeNurseData(){
      return http.postByNoLoading(`/home/AppHomeContactCarePage?versionNo=4.5.8`)
    },
    //获取首页-隐形护理页商品列表
    getHomeNurseGoods(pageIndex, pageSize){
      let url = `/home/GetAppHomeContactCareGoodsByPage?versionNo=4.5.8&pageSize=${pageSize}&pageIndex=${pageIndex}`
      if(pageIndex == 1){
        return http.postByNoLoading(url)
      }
      return http.post(url)
